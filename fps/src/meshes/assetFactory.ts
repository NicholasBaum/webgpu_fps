...
export function CREATE_CUBE_w_NORMALS(material?: Material): ModelAsset {
...
export function CREATE_CUBE(material?: Material): ModelAsset {
    return new ModelAsset(
        "cube_asset",
        CUBE_VERTEX_ARRAY,
        CUBE_VERTEX_COUNT,
...
export function CREATE_SPHERE(numSegments: number, smoothNormals: boolean = true, material?: Material): ModelAsset {
    return new ModelAsset(
        "sphere_asset",
...
export function CREATE_CYLINDER(n_sides: number = 5, smoothNormals: boolean = false, material?: Material): ModelAsset {
    const [rin, rout, height] = [0.7, 1.5, 3.0];
    return new ModelAsset(
        "cylinder_asset",
